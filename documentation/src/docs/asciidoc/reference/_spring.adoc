...
This artifact provides a Spring Boot auto-configuration class to automatically register a link:{apidir}/com/holonplatform/jaxrs/spring/boot/JaxrsClientBuilder.html[JaxrsClientBuilder^] bean, which can be used to obtain a configured JAX-RS `Client` instance.

To customize the JAX-RS `ClientBuilder` used to obtain the client instances, the link:{apidir}/com/holonplatform/jaxrs/spring/boot/JaxrsClientCustomizer.html[JaxrsClientCustomizer^] interface can be used: any Spring bean which implement the `JaxrsClientCustomizer` interface will be auto-detected and the `customize(ClientBuilder clientBuilder)` method will be invoked when a `ClientBuilder` is created.

To replace the default `ClientBuilder` instance lookup/creation strategy, a link:{apidir}/com/holonplatform/jaxrs/spring/boot/JaxrsClientBuilderFactory.html[JaxrsClientBuilderFactory^] bean type can be declared in Spring context, which will be used by the `JaxrsClientBuilder` to create a new `ClientBuilder` instance.
