...
* Improved *Swagger* API documentation listing endpoints *auto-configuration* when using the _Spring Boot_ integration. The JAX-RS `@Path` endpoint classes annotated with the Swagger `@Api` annotation are automatically detected and included in the Swagger API documentation listing endpoints. See <<SwaggerAPIEndpointAutoConfiguration>>.
