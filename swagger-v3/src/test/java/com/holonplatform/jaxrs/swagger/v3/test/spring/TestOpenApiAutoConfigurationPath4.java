...
@ActiveProfiles("path4")
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
public class TestOpenApiAutoConfigurationPath4 {
	...
	public void testEndpoint() {
		...
		WebTarget target = client.target("http://localhost:" + port + "/resourcepath4").path("test10");
	...
	public void testOpenApi() {
		final Client client = JerseyClientBuilder.createClient();
		// dft
		Response response = client.target("http://localhost:" + port).path("docs/v3").request().get();
		OpenAPI api = OpenAPIEndpointUtils.readAsJson(response);
		OpenApiValidation.validateTestResourceApi(api, "/resourcepath4", null);
		// json
		response = client.target("http://localhost:" + port).path("docs/v3").queryParam("type", "json").request().get();
		api = OpenAPIEndpointUtils.readAsJson(response);
		OpenApiValidation.validateTestResourceApi(api, "/resourcepath4", null);
		// yaml
		response = client.target("http://localhost:" + port).path("docs/v3").queryParam("type", "yaml").request().get();
