...
public class TestOpenApiAutoConfigurationPath4 {
	...
	public void testOpenApi() {
		final Client client = JerseyClientBuilder.createClient();
		// dft
		Response response = client.target("http://localhost:" + port).path("docs/v2").request().get();
		Swagger api = SwaggerEndpointUtils.readAsJson(response);
		SwaggerValidation.validateTestResourceApi(api, "/resourcepath4", null);
		// json
		response = client.target("http://localhost:" + port).path("docs/v2").queryParam("type", "json").request().get();
		api = SwaggerEndpointUtils.readAsJson(response);
		SwaggerValidation.validateTestResourceApi(api, "/resourcepath4", null);
		// yaml
		response = client.target("http://localhost:" + port).path("docs/v2").queryParam("type", "yaml").request().get();
