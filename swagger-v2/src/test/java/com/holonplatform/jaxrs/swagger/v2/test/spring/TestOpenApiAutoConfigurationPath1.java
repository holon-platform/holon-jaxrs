...
@ActiveProfiles("path1")
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
public class TestOpenApiAutoConfigurationPath1 {
	...
	public void testEndpoint() {
		...
		WebTarget target = client.target("http://localhost:" + port + "/resourcepath1").path("test10");
	...
	public void testOpenApi() {
		final Client client = JerseyClientBuilder.createClient();
		// dft
		Response response = client.target("http://localhost:" + port).path("docs").request().get();
		Swagger api = SwaggerEndpointUtils.readAsJson(response);
		SwaggerValidation.validateTestResourceApi(api, "/resourcepath1", null);
		// json
		response = client.target("http://localhost:" + port).path("docs").queryParam("type", "json").request().get();
		api = SwaggerEndpointUtils.readAsJson(response);
		SwaggerValidation.validateTestResourceApi(api, "/resourcepath1", null);
		// yaml
		response = client.target("http://localhost:" + port).path("docs").queryParam("type", "yaml").request().get();
