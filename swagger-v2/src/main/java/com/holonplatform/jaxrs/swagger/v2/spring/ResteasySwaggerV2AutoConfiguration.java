 ...
 * implementation). The {@link ApiConfiguration} annotation can be used on each API configuration
 * bean to setup the API listing endpoint, for example to declare the JAX-RS endpoint path. When
 * more then one {@link SwaggerConfig} type bean is declared, the
 * {@link ApiConfiguration#contextId()} attribute should be used to declare a different API context
 * id for each configuration.</li>
 * <li>When no {@link SwaggerConfig} type bean is defined, the
...
public class ResteasySwaggerV2AutoConfiguration extends AbstractSwaggerV2AutoConfiguration<ResteasyConfig> {
	 ...
	 * (non-Javadoc)
	 * @see com.holonplatform.jaxrs.swagger.v3.internal.spring.AbstractSwaggerV3AutoConfiguration#
	 * registerEndpoint(javax.ws. rs.core.Application, java.lang.Class)
	 */
	@Override
